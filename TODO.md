# TODO — Manuel I : Prologue

## Priorité immédiate

- [ ] Finaliser le titre du Manuel I : *L'Héritage du Wuju* reste le titre de travail.
- [x] Rédiger les pages d'introduction destinées au MJ.
- [x] Écrire le synopsis général du Prologue.
- [x] Verrouiller la version narrative du chapitre I.
- [x] Verrouiller la version narrative du chapitre II.
- [x] Réécrire le chapitre III pour le Temple Wuju souterrain.
- [x] Synchroniser les chapitres II et III en modules Homebrewery.
- [x] Poser les annexes techniques V1.
- [x] Poser les récompenses de Doran.
- [x] Poser l'arbre de progression Wuju.
- [x] Poser les fiches PNJ principales.
- [x] Créer les handouts joueurs principaux.
- [x] Préparer une version Homebrewery imprimable des handouts.
- [x] Assembler les modules dans une version finale unique `homebrewery/prologue-complet.md`.

## Chapitre I — Les Épreuves du Wuju

### Terminé

- [x] Baie des Pins.
- [x] Route vers le monastère.
- [x] Jun et Maître Yi.
- [x] Thé au dojo.
- [x] Deux épreuves sur trois.
- [x] Esprit Rancunier.
- [x] Escorte du Manuscrit.
- [x] Sanctuaire sous la Falaise.
- [x] Reliquaire Wuju.
- [x] Souffle intérieur.
- [x] Arrivée de Cassian.
- [x] Transition vers les Chutes.
- [x] Gardien de jade mineur.
- [x] Esprit Rancunier jouable techniquement.
- [x] Pont, horloge d'effondrement et pillards de l'Escorte du Manuscrit.
- [x] Progression niveau 1 → 2.

### Restant

- [x] Ajouter les renvois vers les annexes.
- [x] Fixer les cartes du chapitre I : aucune battlemap obligatoire en V1 ; carte régionale facultative.
- [x] Illustration maîtresse chapitre I : Master Yi officiel LoR intégré.
- [ ] Faire la passe éditoriale finale Homebrewery.

## Chapitre II — Les Chutes de Brume

### Terminé

- [x] Jun accompagne Yi, Cassian et les PJ.
- [x] Vision de la statue sans visage.
- [x] PCW 0 à 5.
- [x] Sentier des Brumes.
- [x] Cairns déplacés sans révéler Kayn.
- [x] Bassin du Silence.
- [x] Trois postures Wuju.
- [x] Épée et mémoire de Maître Urong.
- [x] Royaume Spirituel.
- [x] Enseignement Wuju mineur par PJ.
- [x] Ruines et sceau silence / souffle / relâchement.
- [x] Gardien des Chutes harmonisé : CA 15, 45 PV, FP 2 de référence.
- [x] Seigneurs des Darkin.
- [x] Première manifestation claire de Xolaani.
- [x] Retour des lames au temple comme trajectoire principale.
- [x] Aucun combat contre Kayn.
- [x] Retour calme et silence anormal.
- [x] Module Homebrewery complet.
- [x] Handouts du poème d'Urong, des trois postures et de la vision.

### Restant

- [x] Ajouter davantage de murmures personnalisés de Xolaani selon les PJ.
- [ ] Transférer puis intégrer la carte `Sanctuaire caché sous la cascade` dans la mise en page. Source retrouvée et copie web préparée.
- [x] Ajouter les renvois vers le bestiaire et les annexes.
- [ ] Faire la passe éditoriale finale.

## Chapitre III — L'Assaut du Temple

### Terminé

- [x] Temple Wuju creusé dans la montagne.
- [x] Architecture simplifiée à trois niveaux jouables + grottes comme connexion.
- [x] PJ au niveau intermédiaire au début de l'assaut.
- [x] Infiltration simultanée depuis le haut et les grottes.
- [x] Yi contre Kayn au niveau supérieur.
- [x] Choix monter / tenir / descendre.
- [x] Récupération des armes.
- [x] Révélation du magnétisme de Cassian.
- [x] Transfuge noxien comme boss tactique intermédiaire.
- [x] Rhaast reconnaît Xolaani.
- [x] Tentation de Jun.
- [x] Combat final contre Kayn au reliquaire.
- [x] Victoire par dégâts ou trois objectifs.
- [x] Kayn à 52 PV au début du final, retraite à 25 PV.
- [x] Ajustement : 1 action d'élite/round contre 3 PJ, 2 contre 4–5.
- [x] Épilogue à l'aube.
- [x] Version Homebrewery synchronisée.
- [x] Direction visuelle des niveaux du temple validée.
- [x] Handout des usages du Temple Wuju.

### Restant

- [ ] Produire/exporter les versions définitives MJ et joueurs des cartes.
- [ ] Ajouter les cartes dans Homebrewery.
- [x] Ajouter les renvois explicites au bestiaire.
- [ ] Faire une passe de test réelle du rythme de l'assaut.
- [x] Illustration maîtresse chapitre III : Kayn officiel LoR intégré.

## Annexes et règles

- [x] Points de Confiance Wuju.
- [x] Brume protectrice.
- [x] Souffle intérieur.
- [x] Feuille sans vent.
- [x] Lame posée.
- [x] Souffle du bassin.
- [x] Regard d'Urong.
- [x] Pas dans la brume.
- [x] Arbre Wuju V1.
- [x] Gardien mineur.
- [x] Gardien des Chutes.
- [x] Transfuge.
- [x] Éclaireurs / Lame d'ombre.
- [x] Profil d'objectif de Kayn.
- [x] Cassian en combat.
- [x] Marques de Xolaani.
- [x] Objets de Doran : Lame, Bouclier, Anneau.
- [x] États de fin du Prologue.
- [x] Fiches PNJ : Yi, Jun, Cassian, Urong, Kayn, Rhaast, Xolaani.
- [x] Handouts joueurs : vision, poème, postures, techniques, usages du temple, objets de Doran.
- [ ] Définir les fiches techniques éventuelles de Yi et Jun uniquement si un test de jeu montre qu'elles sont nécessaires.

## Cartes

- [x] Architecture générale du Temple Wuju validée.
- [x] Carte du niveau supérieur — Temple principal.
- [x] Carte du niveau intermédiaire — Quartiers.
- [x] Carte du niveau inférieur — Reliquaire.
- [x] Carte de référence existante des Chutes validée.
- [ ] Export propre des cartes du temple avec nomenclature stable.
- [ ] Versions sans informations MJ.
- [ ] Carte régionale minimale du chapitre I si réellement utile.

## Phase visuelle

- [x] Plan d'intégration visuelle V1.
- [x] Registre des sources d'images.
- [x] Identifier Jun, Ting, Disciple de Doran, Wuju Style, Master Yi et Mistfall dans LoR.
- [x] Préparer les URLs Riot Data Dragon et le script de collecte HD.
- [x] Finaliser les crédits artistes / sources pour les arts déjà intégrés.
- [x] Insérer Yi, Jun, Kayn et Xolaani dans Homebrewery.
- [ ] Finaliser la couverture originale.
- [ ] Finaliser les cartes joueurs / MJ.
  - [x] Niveau supérieur joueurs intégré dans Homebrewery.
  - [ ] Niveau supérieur MJ.
  - [x] Niveau intermédiaire joueurs intégré.
  - [ ] Niveau intermédiaire MJ.
  - [x] Niveau inférieur joueurs validé visuellement.
  - [ ] Niveau inférieur : transfert binaire GitHub + intégration Homebrewery. Copie web préparée.
  - [ ] Niveau inférieur MJ.
  - [x] Vue générale simplifiée joueurs / MJ en SVG.
  - [ ] Chutes joueurs / MJ — source joueurs validée et copie web préparée.

## Identité graphique

- [x] Palette de base Wuju / Ionia définie dans le CSS.
- [x] Styles distincts : lecture, note MJ, Wuju, Darkin, rencontre.
- [ ] Couverture du Prologue.
- [x] Illustration maîtresse chapitre I — Master Yi.
- [x] Illustration maîtresse chapitre II — Jun.
- [x] Illustration maîtresse chapitre III — Kayn.
- [ ] Passe graphique finale sur le sommaire et la couverture.

## Passe finale

- [x] Assembler tous les modules Homebrewery.
- [ ] Vérifier les sauts de page et colonnes.
- [ ] Supprimer les répétitions entre chapitre et annexes.
- [x] Vérifier les constantes principales DD/PV et ajouter `scripts/check_canon.py`.
- [ ] Harmoniser les voix des PNJ.
- [x] Ajouter crédits / notice fan Riot définitive.
- [ ] Exporter une première version PDF de test.
- [ ] Relire le PDF page par page.

## Mémoire du projet

À chaque décision importante :

1. mettre à jour `CANON.md` ou le fichier concerné pour les faits de scénario ;
2. mettre à jour `DECISIONS.md` pour les choix de conception ;
3. mettre à jour `STATUS.md` pour l'état de reprise rapide ;
4. cocher cette feuille de route.
